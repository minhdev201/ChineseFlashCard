...
export function FlashcardTab({
  ...
  const bucketCounts = useMemo(() => {
    ...
    vocab.forEach((item) => {
      if (counts[item.memory_bucket] !== undefined) {
        counts[item.memory_bucket]++;
      }
    });
    return counts;
  }, [vocab]);
      ...
      <div className="relative z-20 bg-white rounded-3xl p-4 sm:p-5 border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto md:ml-auto">
            ...
            <div className="relative shrink-0" ref={bucketDropdownRef}>
              <button
              ...
              >
                ...
                <span>{sourceLabel(bucketFilter)}</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-[11px] font-mono px-1.5 py-0.5 rounded-md ${
                          isActive ? 'bg-indigo-100/80 text-indigo-700 font-bold' : 'bg-slate-100 text-slate-500'
                        }`}>
                          {count}
                        </span>
                        {isActive && <Check className="w-3.5 h-3.5 ml-1 text-indigo-500 shrink-0" />}
                      </button>
                    );
                  })}
                </div>
            ...
            <button
              ...
              className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all shadow-sm active:scale-95 ${
                sortMode === 'random'
            ...
            <button
              ...
              className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-colors ${autoSpeak
