...
import type { ReviewRating, Vocab } from '@/lib/types';
...
interface FlashcardTabProps {
  vocab: Vocab[];
  onReview: (id: string, rating: ReviewRating) => Promise<void>;
...
const RATING_BUTTONS: {
  ...
  { rating: 'again', label: 'Quên', hint: 'Lặp lại', classes: 'bg-rose-500 hover:bg-rose-600 text-white' },
  { rating: 'hard', label: 'Khó', hint: 'Ngắn', classes: 'bg-orange-500 hover:bg-orange-600 text-white' },
  { rating: 'good', label: 'Tốt', hint: 'Trung bình', classes: 'bg-emerald-500 hover:bg-emerald-600 text-white' },
  { rating: 'easy', label: 'Dễ', hint: 'Dài hạn', classes: 'bg-blue-500 hover:bg-blue-600 text-white' },
];

export function FlashcardTab({ vocab, onReview, filterDueOnly, onClearDueFilter }: FlashcardTabProps) {
  const baseList = useMemo(() => {
    const due = vocab.filter(isDue);
    if (filterDueOnly && due.length > 0) return due;
    return vocab.length > 0 ? vocab : [];
  }, [vocab, filterDueOnly]);
  ...
  const prevIdSig = useRef<string>('');

  useEffect(() => {
    const sig = baseList.map((w) => w.id).join(',');
    if (sig !== prevIdSig.current) {
      prevIdSig.current = sig;
      setQueue(baseList);
      setIndex(0);
      setFlipped(false);
      setInput('');
      ...
      setQueue((prev) =>
        prev.map((card) => baseList.find((w) => w.id === card.id) || card)
      );
    }
  }, [baseList]);
  ...
  }, [flipped, handleCheck, handleRating, goNext, goPrev]);
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-slate-500 mb-6">
          Hãy thêm từ mới ở tab "Thêm từ", hoặc đợi đến ngày ôn tập tiếp theo.
        </p>
        {filterDueOnly && (
          <button
        ...
        >
          ...
          <div className="flip-face absolute inset-0 rounded-3xl bg-white border border-slate-200 shadow-xl flex flex-col items-center justify-center p-6 cursor-pointer">
            ...
            <div className="absolute top-4 left-4">
              <span
                className={`text-xs px-2 py-1 rounded-full font-medium ${masteryColor(current.srs_level)}`}
              >
                {masteryLabel(current.srs_level)}
              </span>
            </div>
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          ...
          {!flipped ? (
            ...
            >
              <Check className="w-5 h-5" />
              <span className="hidden sm:inline">Tốt</span>
            </button>
          )}
        </div>
        {feedback === 'correct' && (
      ...
      <div className="grid grid-cols-4 gap-2 sm:gap-3 mb-4">
        {RATING_BUTTONS.map((b) => (
          <button
            ...
            className={`flex flex-col items-center py-3 rounded-xl font-semibold transition-all ${b.classes} hover:scale-[1.02] active:scale-95`}
          >
            <span className="text-sm">{b.label}</span>
            <span className="text-[10px] opacity-80 hidden sm:block">{b.hint}</span>
          </button>
        ))}
      </div>
